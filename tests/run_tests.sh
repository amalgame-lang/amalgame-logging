#!/bin/bash
# ─────────────────────────────────────────────────────
#  amalgame-logging — Test Runner
#  Usage: ./tests/run_tests.sh [/path/to/amc]
#
#  Stages a fake package cache pointing at the local working tree,
#  pre-builds the per-package facade archive, then compiles + links
#  + runs the test sample, asserting the expected [PASS] lines
#  appear on stdout.
#
#  Requires amc 0.7.7+ for the cross-class external forward decl fix.
# ─────────────────────────────────────────────────────
...
# ── Setup ──────────────────────────────────────────────
BUILD_DIR="$(mktemp -d -t aflog-tests-XXXXXX)"
...
echo ""
echo "════════════════════════════════════════════"
echo "  amalgame-logging — Test Suite"
echo "════════════════════════════════════════════"
...
PKG_GIT="github.com/amalgame-lang/amalgame-logging"
...
if [ -e "$WORK_BUILD_DIR" ] && [ ! -L "$WORK_BUILD_DIR" ]; then
    rm -rf "$WORK_BUILD_DIR"
fi
rm -f "$WORK_BUILD_DIR"
ln -s "$FACADE_BUILD_DIR" "$WORK_BUILD_DIR"
ARCHIVE="$FACADE_BUILD_DIR/libamalgame-pkg-Log.a"

echo "── Pre-compiling facade.am → libamalgame-pkg-Log.a ──"
"$AMC" --lib --quiet "$PKG_ROOT/facade.am" -o "$FACADE_BUILD_DIR/Log-facade"
gcc -O2 -I"$AMC_RUNTIME" -w -c "$FACADE_BUILD_DIR/Log-facade.c" \
    -o "$FACADE_BUILD_DIR/Log-facade.o"
ar rcs "$ARCHIVE" "$FACADE_BUILD_DIR/Log-facade.o"
...
run_test() {
    ...
    cp "$SCRIPT_DIR/stdlib_logging.am" "$PROJ_DIR/test.am"
...
run_test "level debug"                  "[PASS] level debug"
run_test "level info"                   "[PASS] level info"
run_test "level warn"                   "[PASS] level warn"
run_test "level error"                  "[PASS] level error"
run_test "level case-insensitive"       "[PASS] level case-insensitive"
run_test "level unknown falls to info"  "[PASS] level unknown falls to info"
run_test "file set"                     "[PASS] file set"
run_test "file unset"                   "[PASS] file unset"
run_test "level filter via file"        "[PASS] level filter via file"
run_test "file labels"                  "[PASS] file labels"
